...
public class QuartzScheduler implements RemotableQuartzScheduler {
    ...
    public void scheduleJobs(List<JobDetail> jobs, Trigger trigger, boolean replace) throws SchedulerException {
        validateState();
        Objects.requireNonNull(trigger, "Trigger must not null.");

        List<JobKey> jobKeys = new ArrayList<>();
        for (JobDetail jobDetail : jobs) {
            jobKeys.add(jobDetail.getKey());
        }
        JobKey jobKey = new JobKey(jobKeys);
        jobKey.setJobKeyList(jobKeys);
        jobKey.setJobPack(true);

        OperableTrigger opt = (OperableTrigger) trigger;
        opt.setJobKey(jobKey);
        opt.validate();
