...
public abstract class JobStoreSupport implements JobStore, Constants {
    ...
    protected void storeJobs(Connection conn,
                             List<JobDetail> newJobs, boolean replaceExisting)
            throws JobPersistenceException {
        for (JobDetail newJob : newJobs) {
            storeJob(conn, newJob, replaceExisting);
        }
    }
     ...
     * @param conn
     * @param newTrigger
     * @param job             如果是打包作业则该参数为null，需要自行查询Job列表进行处理
    ...
    protected void storeTrigger(Connection conn,
                                ...
                                boolean replaceExisting, String state,
        ...
        try {
            ...
            List<JobDetail> packJobList = new ArrayList<>();
            if (job == null) {
                // 检索作业，打包可能会检索多个作业
                if (jobKey.isJobPack()) {
                    for (JobKey packJobKey : jobKey.getJobKeyList()) {
                        JobDetail packJob = retrieveJob(conn, jobKey);
                        packJobList.add(packJob);
                    }
                } else {
